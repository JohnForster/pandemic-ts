...
const App: React.FC = () => {
  ...
  const save = () => {
    copyStringToClipboard(`module.exports = ${JSON.stringify(gameData.cities.map(c => c.data), null, 2)}`)
  }
  ...
  return (
    <Styled.App>
      ...
      <button onClick={toggleDev}>Toggle dev mode</button><br/>
      Dev mode: {isDev ? 'ON ' : 'OFF'}<br/>
      Selected: {dev.selectedId ?? 'none'}
      <button onClick={save}>Save</button><br/>
      {Array(8).fill(null).map((x, i) => (
